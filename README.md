...
* Multi-Thread (Pthread): `pthread_create`를 사용하여 요청당 스레드를 생성하며, 힙 메모리 할당을 통해 스레드 간 데이터 무결성을 보장.
* Automated Build: `Makefile`의 매크로 정의(`-D USE_THREAD`)를 활용해 코드 수정 없이 두 가지 모델을 각각 컴파일 가능하게 함.
